...
**Date:** 2026-04-10
**Updated:** 2026-04-21 (security/usability review pass 2)
...
public class JWTDecoder {
    private final JSONProcessor jsonProcessor;
    private final Duration clockSkew;           // default: Duration.ZERO
...
| `clockSkew` | `Duration.ZERO` | Strict by default; matches jjwt and auth0 java-jwt. Nimbus defaults to 60s — no industry consensus. Callers enable skew explicitly. |
...
No maximum is imposed on `clockSkew` — callers are responsible for choosing reasonable values. RFC 8725 §3.3 recommends "a few seconds to a minute" for real deployments.
...
| `JWTDecoder.java` | Use `JSONProcessor`, `Instant` for time validation, `Duration` for clock skew, constructor-based config (final fields), add `decodeUnsecured()`, simplified overloads, optional validator, `crit` header validation, `expectedType`/`expectedAlgorithms`/size/depth/dup-key config via builder |
...
- // RFC 8725 §2.2 - HMAC-with-RSA-public-key (cross-algorithm) attack rejected
- // RFC 8725 §2.3 - signature stripping rejected (MissingSignatureException)
...
- // RFC 8725 §3.2 - HS256 with 31-byte secret rejected (InvalidKeyLengthException)
- // RFC 8725 §3.5 - RSA 1024-bit key rejected (InvalidKeyLengthException)
- // RFC 8725 §3.8 - expectedType accepts matching typ
- // RFC 8725 §3.8 - expectedType rejects mismatched typ
- // RFC 8725 §3.11 - verifier per algorithm family (binding enforced via canVerify)
...
|-----------|-------|------------------|
| §2.1 | "none" algorithm attack | No built-in verifier handles `alg: none`. The decoder throws `MissingVerifierException` unless the caller explicitly wires a verifier for it. See [Section 1](#1-algorithm-interface) "Security: the 'none' algorithm". |
| §2.2 | HMAC algorithm confusion (public key as HMAC secret) | All built-in `Verifier` implementations have `canVerify()` that returns true only for their own algorithm family. RSA/EC/EdDSA verifiers reject HS* header values. HMAC verifier's `verify()` then fails on the forged signature. Tested via `VulnerabilityTest.test_vulnerability_HMAC_forgery` ported to 7.0 API. |
| §2.3 | Cross-JWT token confusion | Addressed via `expectedType` (RFC 8725 §3.8, §3.11) and `expectedAlgorithms` (§3.1). Callers can enforce explicit type and algorithm per usage site. |
| §3.1 | Perform algorithm verification | Decoder checks `verifier.canVerify(header.alg)` before calling `verify()`. Optional `expectedAlgorithms` set provides defense-in-depth whitelist. |
| §3.2 | Use appropriate algorithms | All standard algorithms in [Section 1](#1-algorithm-interface) are from the IANA JOSE registry. Deprecated `"EdDSA"` identifier is not a standard constant per RFC 9864. No support for null/none, `A256CBC-HS512` without authenticated encryption, or other dubious constructs. |
| §3.3 | Validate cryptographic inputs | Base64URL strictness (see [Section 5](#5-encoder--decoder)) rejects malformed segments. `LatteJSONProcessor` rejects duplicate keys and enforces depth limits. Token size is capped. |
| §3.4 | Ensure cryptographic keys have sufficient entropy | Library enforces minimum key sizes (see [Section 6](#6-signer--verifier) "Key Length and Strength Enforcement"). **Caller responsibility:** key entropy itself cannot be measured; use `JWTUtils.generate*()` helpers which use `SecureRandom`. |
| §3.5 | Avoid compression of encrypted data | N/A — JWE is out of scope for 7.0. |
| §3.6 | Use UTF-8 | All JSON and base64url decoding uses UTF-8 explicitly via `StandardCharsets.UTF_8`. |
| §3.7 | Validate issuer and subject | **Caller responsibility** — use the post-decode `Consumer<JWT> validator` parameter to check `iss`/`sub` per application policy. |
| §3.8 | Use explicit typing | Encoder defaults `typ` to `"JWT"`. Decoder supports `expectedType()` for applications that want to pin `typ` to a specific value (e.g., `"at+jwt"` per RFC 9068, `"dpop+jwt"` per RFC 9449). |
| §3.9 | Use mutually exclusive validation rules for different kinds of JWTs | Supported via `expectedType` + `expectedAlgorithms` combination. Different decoder instances can be configured with mutually-exclusive rule sets for access tokens vs ID tokens vs DPoP proofs. |
...
| §3.11 | Use UTF-8 / identify critical vulnerabilities | `crit` header enforcement per RFC 7515 §4.1.11 — unknown critical parameters rejected unless opted in via `criticalHeaders`. See [Section 3](#3-header-immutable--builder). |
| §3.12 | Avoid mix-up attacks | **Caller responsibility** — application-level decision. Library provides `expectedType`, `expectedAlgorithms`, and `iss`/`aud` access for the caller to enforce. |
