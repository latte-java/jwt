...
public class JWTTest {
  ...
  public void decode_validator_exception_propagates() {
    ...
    InvalidJWTException thrown = expectThrows(InvalidJWTException.class, () ->
        JWT.decode(
            ...
            j -> {
              throw new InvalidJWTException("rejected by validator");
            }));
  ...
  public Object[][] fromMapMalformed() {
    return new Object[][]{
        ...
        {"jti", List.of("x")},
        // Aud with object
        {"aud", new LinkedHashMap<String, Object>() {{
          put("foo", 1);
        }}},
  ...
  /**
   * The public Builder does not expose a header() setter (header is populated by the decoder). For equality tests we go
   * through {@link JWT#fromMap} to obtain a JWT with a populated header.
   */
