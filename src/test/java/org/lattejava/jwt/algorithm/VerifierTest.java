...
public class VerifierTest {
  ...
  static {
    try {
      ...
      String hmacToken = JWT.getEncoder().encode(jwt, HMACSigner.newSHA256Signer(hmacSecret));
      ...
      String rsaToken = JWT.getEncoder().encode(jwt, RSASigner.newSHA256Signer(rsaPrivateKey));
      algorithms.add(new Pair<>(new Pair<>(RSASigner.newSHA256Signer(rsaPrivateKey), RSAVerifier.newVerifier(rsaPublicKey)), rsaToken));

      // RSA-PSS
      String rsaPssToken = JWT.getEncoder().encode(jwt, RSAPSSSigner.newSHA256Signer(rsaPrivateKey));
      ...
      String ecToken = JWT.getEncoder().encode(jwt, ECSigner.newSHA256Signer(ecPrivateKey));
      ...
      String ed25519Token = JWT.getEncoder().encode(jwt, EdDSASigner.newSigner(ed25519PrivateKey));
  ...
  public void verify() {
    ...
    for (Pair<Pair<Signer, Verifier>, String> algorithm : algorithms) {
      ...
      try {
        JWT jwt = JWT.getDecoder().decode(algorithm.second(), algorithm.first().second());
        assertNotNull(jwt);
