...
public final class JWT {
   ...
   * {@link VerifierResolver}. Signature verification runs BEFORE payload deserialization so a malformed payload cannot
   ...
   */
  public static JWT decode(String encodedJWT, VerifierResolver resolver) {
    return JWTDecoder.getDefault().decode(encodedJWT, resolver);
  }

  /**
   * Single-verifier convenience overload. Equivalent to {@code decode(encodedJWT, VerifierResolver.of(verifier))}.
   *
   * @param encodedJWT the compact JWS string; must be non-null
   ...
   * @return the decoded {@link JWT}
   */
  public static JWT decode(String encodedJWT, VerifierResolver resolver, Consumer<JWT> validator) {
   ...
   * validator. The validator runs after signature verification and built-in time validation; implementations throw any
   * {@link JWTException} subclass to reject the token.
   *
   * @param encodedJWT the compact JWS string; must be non-null
   * @param decoder    the JWT decoder; must be non-null
   * @param resolver   the verifier resolver; must be non-null
   ...
   * {@code List} or {@code Map} instances are stored by reference rather than deep-copied. Callers that retain a
   * mutable alias to such a value can observe their later mutations through {@link #getObject(String)},
   * {@link #getMap(String)}, or {@link #getList(String)} on the returned {@link JWT}. To preserve full immutability,
   * callers must not mutate the input map's collection values after this method returns. The same caveat applies to
   * {@code Builder.claim}.</p>
   */
   ...
   * <p>
   * The audience claim identifies the recipients that the JWT is intended for. On the wire this may be an array of
   * strings or a single string; any string values containing a {@code :} must be URIs. This accessor always returns a
   * list (empty if the claim is absent); the {@link AudienceSerialization} mode selects the serialized form and is
   * available via {@link #audienceSerialization()}.
   */
  ...
  /**
   * The {@link AudienceSerialization} mode for this JWT, or {@code null} when {@code aud} is absent.
   */
  ...
  /**
   * Returns all claims (registered + custom) as Java-typed values — timestamps are returned as {@link Instant}, not as
   * epoch-seconds. Suitable for callers reading claim state; for JSON serialization use {@link #toSerializableMap()}
   * which emits timestamps as NumericDate.
   */
  ...
  /**
   * Returns true if the claim fields of this JWT equal the claim fields of the other JWT. The {@link Header} is
   * intentionally not consulted, and the {@link AudienceSerialization} mode is intentionally ignored - audience is
   ...
   * processing. Serialized as NumericDate (seconds since Epoch).
   */
  ...
  /**
   * Look up a numeric claim as a {@link BigDecimal}. Accepts values already of type {@code BigDecimal},
   * {@code BigInteger}, or any {@link Number} (narrowed via {@code doubleValue()}). Returns {@code null} if absent.
   */
  ...
  /**
   * Look up a numeric claim as a {@link BigInteger}. Accepts values already of type {@code BigInteger},
   * {@code BigDecimal} (truncated), or any {@link Number} (narrowed via {@code longValue()}). Returns {@code null} if
   * absent.
   */
  ...
  /**
   * Look up a numeric claim and narrow to {@code double} via {@link Number#doubleValue()}; returns {@code null} if
   * absent.
   */
  ...
  /**
   * Look up a numeric claim and narrow to {@code float} via {@link Number#floatValue()}; returns {@code null} if
   * absent.
   */
  ...
  /**
   * Look up a claim as a list of a specific element type. Returns {@code null} if absent; throws
   * {@link ClassCastException} on the first element that is not an instance of {@code elementType}. Null elements are
   * permitted.
   */
  ...
  /**
   * Look up a numeric claim as a raw {@link Number}. Returns {@code null} if absent; throws if the claim is not a JSON
   * number.
   */
  ...
  /**
   * Returns {@code true} if {@code value} appears in the {@code aud} claim. {@code null} input returns {@code false}.
   */
  ...
  /**
   * The decoded JWT header. This is not considered part of the JWT payload, but is attached here for caller
   * convenience.
   */
   ...
   * <p>
   * The issued at claim identifies the time at which the JWT was issued. Serialized as NumericDate (seconds since
   * Epoch).
   */
   ...
   * <p>
   * The issuer claim identifies the principal that issued the JWT. If the value contains a {@code :} it must be a URI.
   */
   ...
   * <p>
   * This claim identifies the time before which the JWT MUST NOT be accepted for processing. Serialized as NumericDate
   * (seconds since Epoch).
   */
   ...
   * <p>
   * The subject claim identifies the principal that is the subject of the JWT. If the value contains a {@code :} it
   * must be a URI.
   */
  ...
  /**
   * Returns a JSON-serializable view of the claims. Timestamps ({@code exp}, {@code nbf}, {@code iat}) are emitted as
   * NumericDate (epoch seconds) per RFC 7519 §2, and {@code aud} is emitted as either a single string or an array to
   * match the recorded {@link AudienceSerialization} mode.
   *
   * @apiNote The returned map is mutable and not shared with the {@code JWT} instance. Callers MUST NOT retain or
   *     mutate it -- the contract is that each call returns a fresh map intended for immediate handoff to a JSON
   *     serializer. The {@code aud} value, when present as an array, references the JWT's internal unmodifiable list
   *     directly; the JSON serializer only iterates it.
   */
  ...
  /**
   * Mutable, reusable builder for {@link JWT}. After {@link #build()} is called, the builder retains its state and may
   * be further modified to produce additional independent {@link JWT} instances; each {@code build()} call produces a
   * fresh immutable instance with an independent copy of any collection fields.
   */
  public static final class Builder {
     ...
     * <p>
     * The audience claim identifies the recipients that the JWT is intended for. On the wire this may be an array of
     * strings or a single string; any string values containing a {@code :} must be URIs.
     * <p>
     * This overload sets a single-element audience. Serialization defaults to
     * {@link AudienceSerialization#ALWAYS_ARRAY}; call {@link #audienceSerialization(AudienceSerialization)} to opt in
     * to {@link AudienceSerialization#STRING_WHEN_SINGLE}.
     */
    ...
    /**
     * Override the serialization form of the {@code aud} claim. Passing {@code null} restores the builder default
     * ({@link AudienceSerialization#ALWAYS_ARRAY}). Has no effect when the audience is absent.
     */
    ...
    /**
     * Add a claim. If the name matches a registered claim (iss, sub, aud, exp, nbf, iat, jti), the value is routed to
     * the corresponding typed setter with type coercion. A value that cannot be coerced throws
     * {@link IllegalArgumentException}. Unrecognized names are stored in {@code customClaims}.
     */
     ...
     * Registered Claim {@code iss} as defined by RFC 7519 §4.1.1. Use of this claim is OPTIONAL.
     * <p>
     * The issuer claim identifies the principal that issued the JWT. If the value contains a {@code :} it must be a
     * URI.
     */
     ...
     * Registered Claim {@code nbf} as defined by RFC 7519 §4.1.5. Use of this claim is OPTIONAL.
     * <p>
     * This claim identifies the time before which the JWT MUST NOT be accepted for processing. Serialized as
