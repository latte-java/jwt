...
This project is built with the Latte CLI (`latte`), not Maven and not Savant. The authoritative build is `project.latte`; `.github/workflows/test.yml` drives the same targets with `latte clean int`. Compile target is Java 21.
...
- **Decoding uses a `VerifierResolver`.** `VerifierResolver.of(verifier)` for a single key; `VerifierResolver.byKid(Map)` for a `kid`-indexed keyring. `JWTDecoder.builder().clockSkew(Duration).fixedTime(Instant).build()` configures clock skew and the decoder's notion of "now" (keep `fixedTime` test-only).
